...
inline Dst clamp_cast(Src v) {
    ...
    if constexpr (std::is_integral_v<Src> && std::is_integral_v<Dst> &&
        ...
        if (v < lo) return std::numeric_limits<Dst>::min();
        if (v > hi) return std::numeric_limits<Dst>::max();
        return static_cast<Dst>(v);
    }
}

template <typename Dst>
void copy_from_linebuf_impl(const ojph::line_buf* line, Dst* dst, size_t width) {
    if ((line->flags & ojph::line_buf::LFT_32BIT) &&
        (line->flags & ojph::line_buf::LFT_INTEGER) == 0) {
        for (size_t i = 0; i < width; ++i)
...
T* decode_to_buffer(ojph::codestream& codestream, const ArrayInfo& info) {
    ...
    if (!data) throw std::runtime_error("Failed to allocate decode buffer");
...
/* Copy one row of the input C buffer into an ojph::line_buf.
   ...
   For 32-bit types, OpenJPH always uses si64 line buffers (precision > 32), which
   copy_to_linebuf_impl handles via the LFT_64BIT branch. */
void copy_row_to_linebuf_c(const void* data, const ArrayInfo& info,
    ...
    if      (bd <= 8  && !sgn) copy_to_linebuf_impl(static_cast<const uint8_t*>(data)  + off, info.width, line);
    else if (bd <= 8)          copy_to_linebuf_impl(static_cast<const int8_t*>(data)   + off, info.width, line);
    else if (bd <= 16 && !sgn) copy_to_linebuf_impl(static_cast<const uint16_t*>(data) + off, info.width, line);
    else if (bd <= 16)         copy_to_linebuf_impl(static_cast<const int16_t*>(data)  + off, info.width, line);
    else if (!sgn)             copy_to_linebuf_impl(static_cast<const uint32_t*>(data) + off, info.width, line);
    else                       copy_to_linebuf_impl(static_cast<const int32_t*>(data)  + off, info.width, line);
}
...
int encode_impl_c(const openjph_array_t* img,
    ...
    try {
...

        {
            ...
            uint32_t clamped = std::min<uint32_t>(
                static_cast<uint32_t>(params->num_decompositions),
        ...
        if (!buf) throw std::runtime_error("Failed to allocate encode output buffer");
...
int decode_impl_c(const uint8_t* codestream_data, size_t codestream_len,
    ...
    try {
        ...
        *out_is_signed = sgn ? 1 : 0;
        ...
        if      (bd <= 8  && !sgn) decoded = decode_to_buffer<uint8_t>(codestream, info);
        else if (bd <= 8)          decoded = decode_to_buffer<int8_t>(codestream, info);
        else if (bd <= 16 && !sgn) decoded = decode_to_buffer<uint16_t>(codestream, info);
        else if (bd <= 16)         decoded = decode_to_buffer<int16_t>(codestream, info);
        else if (!sgn)             decoded = decode_to_buffer<uint32_t>(codestream, info);
        else                       decoded = decode_to_buffer<int32_t>(codestream, info);
...
int openjph_decode(const uint8_t* codestream, size_t codestream_len,
                   ...
                   uint32_t* out_bit_depth, int32_t* out_is_signed,
                   char* err_buf, size_t err_buf_len) {
    return decode_impl_c(codestream, codestream_len,
                         out, out_len, out_ndim, out_dims,
                         out_bit_depth, out_is_signed,
                         err_buf, err_buf_len);
}

void openjph_free(void* ptr) {
    std::free(ptr);
}
